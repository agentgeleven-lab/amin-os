...
export function drawerPlacement(p,width,height){
    const w=Math.max(0,Math.min(448,width-16)),above=Math.max(0,p.y-20),below=Math.max(0,height-p.y-72);
    const upwards=above>=260||above>=below;
    const h=Math.min(720,upwards?above:below);
