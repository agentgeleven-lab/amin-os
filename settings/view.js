...
export function mount(target){
 ...
 function draw(){
  ...
  if(page==='global'){
   ...
   for(const [id,name]of [['map','地图'],['status','状态'],['organizations','势力概览'],['reply','回复选项'],['effects','能力面板'],['information','信息面板'],['worldbooks','世界书管理'],['tts','语音朗读']]){const row=el('label'),check=el('input');check.type='checkbox';check.checked=draft.floor.buttons[id];check.onchange=()=>draft.floor.buttons[id]=check.checked;row.append(check,document.createTextNode('显示'+name+'楼层按钮'));body.append(row);}
