...
export function mountFloorControl(element,id,host,button,document=globalThis.document){
 ...
 const item={id,button,visible:true,open:false,host};row.items.add(item);for(const i of [...row.items].sort((a,b)=>["map","status","characters","inventory","relationships","saves","dice","scene","journal","organizations","reply","effects","information","worldbooks","tts"].indexOf(a.id)-["map","status","characters","inventory","relationships","saves","dice","scene","journal","organizations","reply","effects","information","worldbooks","tts"].indexOf(b.id)))row.toolbar.append(i.button);
