...
export function mountFloorControl(element,id,host,button,document=globalThis.document){
 ...
 const item={id,button,visible:true,open:false,host};row.items.add(item);for(const i of [...row.items].sort((a,b)=>["map","status","reply"].indexOf(a.id)-["map","status","reply"].indexOf(b.id)))row.toolbar.append(i.button);
