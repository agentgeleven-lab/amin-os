...
export const defaultTiles=()=>normalizeTiles(tileDefaults());
const validCell=(x,y,size)=>{const {w,h}=tileSpan(size);return Number.isInteger(x)&&Number.isInteger(y)&&x>=0&&y>=0&&x+w<=TILE_COLUMNS&&y+h<=TILE_MAX_ROWS;};
...
export function canPlaceTile(tiles,id,x,y,size){
 const source=tiles.find(t=>t.id===id);if(!source&&!size)return false;
 const candidate={...source,id,x,y,size:size??source.size};
 return validCell(x,y,candidate.size)&&!tiles.some(t=>t.id!==id&&overlaps(candidate,t));
}
export function normalizeTiles(value){
 ...
 for(const tile of cleaned){if(validCell(tile.x,tile.y,tile.size)&&!placed.some(other=>overlaps(tile,other)))placed.push(tile);else pending.push(tile);}
 for(const tile of pending){
  let found=false;
  for(let y=0;y<TILE_MAX_ROWS&&!found;y++)for(let x=0;x<TILE_COLUMNS&&!found;x++)if(canPlaceTile(placed,tile.id,x,y,tile.size)){tile.x=x;tile.y=y;placed.push(tile);found=true;}
 }
...
export function loadTiles(storage){
 try{
  ...
  if(data){
   const tiles=normalizeTiles(data.tiles),introduced=['characters','inventory','relationships','saves'];
   ...
   const normalized=normalizeTiles(tiles);try{saveTiles(storage,normalized);}catch{}return normalized;
  }
...
export function saveTiles(storage,tiles){storage.setItem(TILE_KEY,JSON.stringify({version:3,knownTargets:tileDefaults().map(t=>t.target),tiles:normalizeTiles(tiles)}));}
export function moveTileToCell(tiles,id,x,y){
 const list=normalizeTiles(tiles),source=list.find(t=>t.id===id);
 if(!source)return {success:false,tiles:list,reason:'missing'};
 if(!validCell(x,y,source.size))return {success:false,tiles:list,reason:'bounds'};
 if(!canPlaceTile(list,id,x,y))return {success:false,tiles:list,reason:'occupied'};
