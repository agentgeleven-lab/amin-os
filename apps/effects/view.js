...
export function mount(target,options={}){
 ...
 const say=text=>{status.textContent=text;status.dataset.state=draftController?'busy':'';};
 const button=(parent,label,fn,primary=false)=>{const b=node('button',label,typeof primary==='string'?primary:primary?'amin-primary':'');b.type='button';b.onclick=async()=>{b.disabled=true;try{await fn();}catch(e){say(e.message);status.dataset.state='error';}finally{b.disabled=false;}};parent.append(b);return b;};
 ...
 function effectForm(effect){
  openForm();const token=api.capture(),store=api.read();body.replaceChildren();const c=card(effect?'调整指令 / 转让':'建立生效记录'),fields=grid(c);let skill,group;
  ...
  const actions=toolbar(c);button(actions,'确认保存',async()=>{await api.save(token,s=>change(s,api.context().chat,effect?'update':'create',{id:effect?.id,skillId:skill?.value,holder:holder.value,targetMode:mode.value,target:mode.value==='direct'?'':targetField.value,scope:scope.value,command:command.value,condition:condition.value}));finish('生效记录已保存');},true);button(actions,'取消',render);
 }
 ...
 function render(){
  ...
  try{const store=api.read(),chat=api.context()?.chat??[];
   if(selected==='能力面板'){renderConsole({body,api,state:consoleState,say,render,manage:()=>{selected='能力管理';render();},groups:groupManager,editSkill:s=>importForm({book:s.book,id:s.entryId,title:s.name,content:s.original,custom:s.custom},s),onFormOpen:openForm,adjust:effectForm,end:endForm,remove:deleteEffect});
    ...
    const effects=activeEffects(store,chat);if(!effects.length)body.append(node('p',store.skills.length?'当前分支没有持续效果。选择能力并确认发动后，会显示在这里。':'先在能力管理中添加能力，再建立生效记录。','amin-empty'));
    for(const e of effects){const c=card(e.skill.name+' · '+effectTarget(e));c.append(node('p',e.paused?'已暂停 · 暂不附加剧情提醒':'生效中 · 后续生成附加提醒','amin-meta'),node('p','持有者：'+e.holder+' ｜ 层面：'+e.scope),node('p','当前指令：'+(e.command||'未指定')),node('p','持续条件：'+e.condition));const actions=toolbar(c);button(actions,'调整 / 转让',()=>effectForm(e));button(actions,e.paused?'恢复效果':'暂停效果',async()=>{const token=api.capture();await api.save(token,s=>change(s,api.context().chat,'pause',{id:e.id,paused:!e.paused}));finish(e.paused?'已恢复效果':'已暂停效果');});button(actions,'分割',()=>splitForm(e));button(actions,'解除',()=>endForm(e));button(actions,'删除生效记录',()=>deleteEffect(e),'amin-danger');}
    ...
    let prompt;try{prompt=compile(store,chat);}catch(e){prompt=e.message;}const preview=card('当前分支 · 提示预览');preview.append(node('p','下次生成将使用以下提醒；超过上限会明确提示，不会截掉部分效果。','amin-meta'),node('pre',prompt||'无提醒','amin-result'));
   }
