...
export function renderConsole({body,api,state,say,render,manage,groups,editSkill,adjust,end,remove,onFormOpen}){
 ...
 const select=(parent,text,choices,value)=>{const l=el('label',text),input=el('select');input.setAttribute('aria-label',text);for(const [id,name]of choices){const o=el('option',name);o.value=id;input.append(o);}input.value=value;l.append(input);parent.append(l);return input;};
 ...
 const chooser=el('section',null,'amin-card amin-ability-chooser');body.append(chooser);
 const group=select(chooser,'能力分组',[['','全部分组'],...names.map(n=>[n,n])],state.group||'');
 const skillSelect=select(chooser,'选择能力',[['','请选择能力'],...choices.map(s=>[s.id,s.name])],state.skillId||'');
 ...
 if(!choices.length)chooser.append(el('p',skills.length?'这个分组没有可展示的能力。可切换分组，或在能力管理中添加／移动能力。':'还没有可展示的能力。可手动创建、从世界书导入，或检查能力的展示开关。'));
 const tools=el('div',null,'amin-toolbar');body.append(tools);
 button(tools,'添加 / 管理能力',manage);if(groups)button(tools,'管理分组',groups);
 button(tools,'刷新能力列表',render);
 const skill=choices.find(s=>s.id===state.skillId);
 let effectList;
 if(skill){
  const ui=appearance(skill),card=el('section',null,'amin-card amin-selected-ability');card.dataset.ability=skill.id;
  card.append(el('h3',ui.icon+' '+skill.name),el('p','分组：'+abilityGroup(skill)));
  ...
  const fields=el('div',null,'amin-target-fields');card.append(fields);
  ...
  const list=el('datalist');list.id='amin-ability-targets-'+crypto.randomUUID();target.setAttribute('list',list.id);
  ...
  body.append(card);
 }else body.append(el('p','先选择分组和能力，再查看说明或主动发动。切换下拉选项不会发动能力。'));
 effectList=el('div');body.append(effectList);
 function refreshEffects(){
  ...
  effectList.append(el('h3','当前效果 · '+current.length));
  if(!current.length)effectList.append(el('p','当前选择还没有持续效果。'));
  for(const e of current){
   ...
   button(actions,'解除',()=>end(e));button(actions,'删除生效记录',()=>remove(e));effectList.append(c);
  }
 }
 refreshEffects();
 function launch(skill,user){
  ...
  const c=el('section',null,'amin-card');body.append(c);
  c.append(el('small',targetMode==='direct'?user+' · 直接发动（无指定对象）':targetName+' ← '+user),el('h3',ui.icon+' '+skill.name));
  ...
  button(c,'确认发动',async()=>{await api.save(capture,s=>change(s,api.context().chat,'create',{skillId:skill.id,holder:user,targetMode,target:targetName,scope:scope.value,command:input.value,condition:condition.value}));render();say('已发动：'+skill.name+(targetMode==='direct'?' · 无指定对象':' → '+targetName));},'amin-primary');
  button(c,'取消并返回能力面板',render);
 }
}
