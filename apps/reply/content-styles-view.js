...
export function mountContentStyles(target, {library, getSelection, setSelection, onChange=()=>{}, document:doc=globalThis.document}) {
    ...
    const row=make('label'),select=make('select');select.setAttribute('aria-label','内容风格');row.append(make('span','内容风格'),select);
    const editor=make('details');editor.append(make('summary','管理内容风格'));
    const list=make('select');list.setAttribute('aria-label','编辑内容风格');
    const name=make('input');name.maxLength=40;name.setAttribute('aria-label','内容风格名称');
    const prompt=make('textarea');prompt.rows=4;prompt.setAttribute('aria-label','内容风格提示词');
    ...
    function render(){
        ...
        const empty=make('option','新建内容风格');empty.value='';list.append(empty);
    ...
    const save=button('保存风格',()=>{
        ...
        drafts.delete(old);library.setDraft(owner,old,false);editing=p.id;drafts.set(p.id,{name:p.name,description:p.description});choose(p.id);disarm();render();note.textContent='内容风格已保存。';
    });
    ...
    const remove=button('删除风格',()=>{
        ...
        const id=editing, draft=drafts.get(id);undo={...library.remove(id),draft};drafts.delete(id);library.setDraft(owner,id,false);editing='';disarm();render();note.textContent='已删除，当前选择安全回退为不附加内容风格。';
    });
