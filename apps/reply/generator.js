...
export async function generateOptions(ctx, settings, { draft = '', world, onContext, signal, isCurrent = () => true } = {}) {
    ...
    const generate=request=>{if(s.nsfwEnabled&&s.nsfwPrompt.trim())request={...request,systemPrompt:request.systemPrompt+'\n用户启用的 NSFW 模式附加要求：\n'+s.nsfwPrompt};return sharedAI?sharedAI.generate('回复选项',ctx,request,{signal,snapshot,data:{card:{userName:data.userName,persona:data.persona,characters:data.characters},books:data.world,chat:data.history,request:request.prompt.slice(0,-('\n参考数据：'+JSON.stringify(data)).length)}}):ctx.generateRaw(request);};
