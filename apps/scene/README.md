...
路径按消息名称、用户标志、正文和 `swipe_id` 计算 SHA-256 指纹，不重复保存正文。读取时只恢复与当前聊天路径匹配的快照；旧版完整正文路径仍可核对，因此重新打开、复制到较早分支、删楼层、编辑正文或切换候选不会把后续场景状态误作过去事实。空聊天操作以空前缀作为故事起点；后续分支继承这类起点事实。读取不修改消息，不改写旧快照，也不自动记录模型轮次。
