...
export function settings(){return {url:'http://127.0.0.1:9883',speed:1,volume:1,seed:42,provider:'azuma',ratio:.5,voicePrompt:'',range:'all',quotes:DEFAULT_QUOTES,...context()?.extensionSettings?.aminTTS};}
export function saveSettings(value){
 const url=endpoint(value.url),seed=Number(value.seed),p=profiles(value);
 if(!['azuma','mimo'].includes(value.provider??'azuma'))throw Error('语音引擎无效');if(![.5,.75].includes(value.ratio??.5))throw Error('索引比例须为 0.5 或 0.75');if((value.voicePrompt??'').length>2000)throw Error('基础声线描述不能超过 2000 字符');
 ...
 async function play(text,source='app',config=settings()){
  ...
  try{emit({phase:'loading',message:'正在连接语音服务…',source});await check(config,signal);if(!alive())return;
   for(let i=0;i<parts.length;i++){
    ...
    clear();objectURL=urls.createObjectURL(blob);audio.src=objectURL;audio.volume=parts[i].volume;audio.playbackRate=config.provider==='mimo'?parts[i].speed:1;audio.preservesPitch=true;
