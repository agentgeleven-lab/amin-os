...
export function createPlayer({audio=new Audio(),request=fetch,check=health,urls=URL}={}){
 ...
 function stop(message='已停止'){epoch++;controller?.abort();controller=null;clear();emit({phase:'idle',message,source:null});}
 async function play(text,source='app',config=settings()){
  stop();const parts=chunks(text);if(!parts.length){emit({phase:'error',message:'没有可朗读的文字'});return;}const ticket=epoch;controller=new AbortController();const signal=controller.signal;
  ...
  try{emit({phase:'loading',message:'正在连接东雪莲服务…',source});await check(config,signal);if(!alive())return;
   for(let i=0;i<parts.length;i++){
    ...
    if(!alive())return;
   }clear();emit({phase:'idle',message:'朗读完成',source:null});
  }catch(error){if(!alive())return;clear();emit({phase:'error',message:error.name==='TimeoutError'?'合成超时，请检查本地服务':error.message+'（请确认本地语音服务已启动）',source:null});}
 }
