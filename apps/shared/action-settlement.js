import { buildTaskSettlement, settlementTasks } from './task-settlement.js';
...
export function actionResources(ctx) {
    ...
    const balances = readInventory(ctx).balances.map(balance => ({ kind: 'balance', balanceId: balance.id, label: `${balance.name}（${balance.ownerId}）`, value: balance.amount }));
    const tasks = settlementTasks(ctx).map(({id,title,status,progress,reward,rewardClaim}) => ({id,title,status,progress,reward,rewardClaimed:!!rewardClaim}));
    return { stats, tasks, rewards: [...stats, ...items, ...balances], costs: [...stats, ...items], rolls: readHistory(ctx).slice().reverse().slice(0, 50).map(roll => ({ id: roll.id, text: roll.text })) };
}
...
export function buildActionSettlement(ctx, input, { operationId, now = new Date().toISOString() } = {}) {
    ...
    if (input.task) rows.push(...buildTaskSettlement(working, input.task, { operationId, now, accept }));
