...
export function buildUnifiedPrompt(ctx, { purpose = 'story', write = purpose === 'story' } = {}) {
    ...
    const writable = write ? selected.filter(adapter => mayWrite(ctx, adapter.id)) : [];
    return [
        '【Amin OS · 统一剧情资料】',
        ...
        ].join('\n\n') : '本次仅供读取，不产生 amin_update 或其他状态更新指令。',
