...
只生成当前一张地图，形态与本次结构示例的 type 一致（graph、hex 或 grid）。不生成整套层级，其他地图由插件保留。格子地图由插件分配空格，路线方位按最终格子位置显示，不锁定道路方位。地点覆盖按本次覆盖程度与用户范围要求执行，不使用固定的少节点上限。先梳理范围内的分区及所有明确提及的地点，再组织连接；结构示例只有两个节点是为了展示字段，不是数量模板。素材不足时允许少量地点，不为凑数虚构地点。优先压缩重复说明而不是随意省略地点，必须返回完整 JSON。
...
export function buildMapGenerationPrompt(document, {nameRoads=false,distanceRoads=false,coverage='full'} = {}) {
...
export function coverageInstruction(value='full'){
 const modes={
  full:'全面覆盖：先按当前层级划分子区域，覆盖素材中所有属于本次范围的明确地点与必要交通枢纽，不能只挑几个知名地标串成一条线。地区图应覆盖各主要城市或分区，城市图应覆盖主要街区与相关设施，建筑内部按实际房间组织；不要混淆层级或把整片地区压成三个地名。允许在规定依据范围内补全真实地点并注明依据。用户明确列出的地点逐项核对，不因示例规模或简洁要求省略。没有依据的地点和连接保持待确认，不承诺穷尽现实世界所有地点。',
  balanced:'适中覆盖：覆盖主要分区、剧情有关地点和必要枢纽，可略去无关次要设施；用户明确要求的地点仍须全部保留，不按固定节点数截断。',
  brief:'简略概览：仅选主要地标和必要交通连接，适合快速概览；用户明确要求的地点不得省略。'
 };
 if(!modes[value])throw Error('请选择有效的地点覆盖程度');
 return '本次地点覆盖：'+modes[value]+'\n只在当前范围内补全；地点数量由范围、资料与用户要求决定，不把交通途经点当作覆盖分区的替代品。';
}
