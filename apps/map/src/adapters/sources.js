...
export async function collectMapSources(ctx,wi,options={}){
 ...
 const books=await collectWorldbooks(ctx,{includeCharacter:true,legacyBindings:true,...options},{...options,check:guard,loadWorldInfo:async()=>wi,readBook:options.readBook??(name=>wi.loadWorldInfo(name)),includeKeys:true});
