...
try{
 ...
 window.h=harness();window.confirm=()=>true;
 window.fakeAI={capture:()=>({}),generate:async(task,ctx,req)=>{const request=JSON.parse(req.prompt);if(request.operation==='assessment')return JSON.stringify({title:'观察榜',criteria:'影响力',summary:'分析',limitations:'资料有限',rows:[{group:'organizations',id:'a',rank:1,assessment:'较强',basis:'资料',strengths:'行政',weaknesses:'未知',confidence:'一般'}]});const d=h.api.read();d.summary='AI当前局势';return JSON.stringify(d);}};
 window.view=await mount(document.getElementById('app'),{api:h.api,ai:()=>fakeAI});window.mountAgain=()=>mount(document.getElementById('app'),{api:h.api,ai:()=>fakeAI});
