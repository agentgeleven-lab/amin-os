...
try{
 ...
 await send('Page.navigate',{url:'http://127.0.0.1:'+server.address().port+'/?sources=1'});
 let loaded=false;for(let i=0;i<100;i++){try{loaded=await evaluate('location.search==="?sources=1"&&document.readyState==="complete"&&!!document.getElementById("app")');}catch{}if(loaded)break;await delay(50);}assert.ok(loaded,'source fixture did not load');
 console.log(await evaluate('(async()=>{const m=await import("/apps/organizations/scripts/unified-browser.mjs");return m.runUnifiedSourceBrowserChecks();})()'));
 if(process.env.AMIN_SOURCES_SCREENSHOT){const shot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});fs.mkdirSync(path.dirname(process.env.AMIN_SOURCES_SCREENSHOT),{recursive:true});fs.writeFileSync(process.env.AMIN_SOURCES_SCREENSHOT,Buffer.from(shot.data,'base64'));}
