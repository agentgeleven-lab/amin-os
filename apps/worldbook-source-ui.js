...
export function mountWorldbookSources(parent,{context,value={},onChange=()=>{},disabled=false,sourceOptions={}}={}){
 ...
 const character=checkbox('读取当前角色设定',state.includeCharacter!==false),enabled=checkbox('是否读取世界书',state.readWorldbooks!==false);
 const note=doc.createElement('p');note.textContent='角色设定只读取角色卡文本；绑定、附加及内嵌世界书请在下方独立勾选。每次AI操作重新读取最新正文；关闭总开关不读取任何书。只读启用条目，忽略关键词等激活策略，不限制素材字符数。';root.append(note);
 ...
 async function refresh(){const own=++revision;report.textContent='正在读取当前启用列表（不读取正文）…';try{const result=await worldbookChoices(context(),{...sourceOptions,check:guard});guard();if(own!==revision)return;choices=result;state.selectedBooks=resolvedBookNames(state,choices);ready=true;report.textContent=choices.length?'当前启用的世界书；仅勾选的书会读取，同名来源自动去重。':'当前没有已启用的世界书。';draw();}catch(e){if(!disposed&&own===revision)report.textContent=e.message;}}
