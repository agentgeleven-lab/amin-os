...
// Run: node scripts/generation-browser-smoke.mjs (AMIN_BROWSER / AMIN_GENERATION_ARTIFACTS optional).
...
const artifacts = process.env.AMIN_GENERATION_ARTIFACTS || path.resolve(root, '../generation-browser-smoke');
...
try {
    ...
    for (const id of ['characters','inventory','relationships','scene','journal']) {
        ...
        assert.equal(await evaluate(`pane('${id}').querySelector('[aria-label="生成范围：人物卡"]').checked`),id==='characters');
        await layoutsAt(id,'generation-entry-'+id,'.amin-generation');
    }
    checks.push('All five app entries mount actual generation form with scoped module selection; 320/390/1280 layouts');
    await app('status');await click('status','联动更新');await expand('status','联合初始化');
    await waitFor(`!!pane('status').querySelector('.amin-generation [aria-label="生成方式"]')`,'joint form');
    await setChecked('status','读取指定聊天楼层',true);await fill('status','生成方式','create');
    const changes=[
        {module:'characters',action:'create-character',target:'gen_alice',data:{name:'阿莉丝',kind:'npc'},reason:'楼层中的同行人'},
        {module:'inventory',action:'create-item',target:'gen_coat',data:{name:'旅行外套',ownerId:'gen_alice',quantity:1},reason:'同行人穿着外套'},
        {module:'relationships',action:'save',target:'gen_relation',data:{fromId:'gen_alice',toId:'pc-a',type:'同行',sources:[0]},reason:'同行'},
        {module:'scene',action:'save-scene',target:'gen_gate',data:{name:'城门',participantIds:['gen_alice','pc-a']},reason:'来到城门'},
        {module:'journal',action:'set_fact',target:'gen_fact',data:{title:'同行到达城门',body:'阿莉丝与艾琳一起到达城门。',sourceStart:0,sourceEnd:1},reason:'所选楼层'},
        {module:'characters',action:'create-character',target:'gen_skip',data:{name:'不采用的人物',kind:'npc'},reason:'可取消建议'}
    ];
    await evaluate('modelOutput='+JSON.stringify({version:1,changes}));
    const before=await evaluate('JSON.stringify(ctx.chatMetadata)');
    await click('status','生成资料草稿');await waitFor(`!!pane('status').querySelector('[aria-label="建议 1 JSON"]')`,'generated editable rows');
    assert.equal(await evaluate('JSON.stringify(ctx.chatMetadata)'),before,'generation only drafts');
    await setChecked('status','采用建议 6 · 人物卡',false);
    changes[0].data.name='阿莉丝·已编辑';await fill('status','建议 1 · 名称',changes[0].data.name);
    await layoutsAt('status','generation-editable-draft','.amin-generation [aria-label="建议 1 · 名称"]');
    await click('status','预览所选变更');await failIfNotice('status','确认保存整组资料');
    assert.equal(await evaluate('JSON.stringify(ctx.chatMetadata)'),before,'preview does not persist');
    await layoutsAt('status','generation-save-preview','.amin-generation pre');
    await click('status','确认保存整组资料');
    await failIfNotice('status','整组资料已保存');
    assert.equal(await evaluate("(await import('/apps/characters/model.js')).readCharacters(ctx).characters.find(p=>p.id==='gen_alice').name"),'阿莉丝·已编辑');
    assert.equal(await evaluate("(await import('/apps/characters/model.js')).readCharacters(ctx).characters.some(p=>p.id==='gen_skip')"),false);
    assert.equal((await readInventory()).items.find(i=>i.id==='gen_coat').ownerId,'gen_alice');
    assert.equal(await evaluate("(await import('/apps/relationships/model.js')).readRelationships(ctx).relationships.find(p=>p.id==='gen_relation').fromId"),'gen_alice');
    assert.ok((await evaluate("(await import('/apps/scene/model.js')).readCurrentScene(ctx).scenes.gen_gate.participantIds")).includes('gen_alice'));
    assert.equal(await evaluate("(await import('/apps/journal/model.js')).currentEntries((await import('/apps/journal/model.js')).readStore(ctx),ctx.chat).some(e=>e.id==='gen_fact')"),true);
    assert.equal(await evaluate("(await import('/apps/linkage/service.js')).getSharedService().settings().enabled"),false,'manual generation keeps automatic linkage disabled');
    assert.equal(await evaluate('ctx.chatMetadata.protected'),'preserve');
    checks.push('Joint generation actual service/adapter/atomic commit creates linked person/item/relation/scene/fact; edits and deselection respected; preview no writes; master remains off');
    // Cancellation uses an abort-aware model stub; actual view and service own abort flow.
    await evaluate('holdModel=true');await click('status','生成资料草稿');await click('status','停止生成');
    await waitFor(`!pane('status').querySelector('.amin-generation button').disabled`,'cancel unlock');await evaluate('holdModel=false');
    checks.push('Actual shared generation view cancellation aborts pending mock model and unlocks controls');
    // A fresh proposal becomes invalid if its selected chat source is edited.
    await evaluate('modelOutput='+JSON.stringify({version:1,changes:[{module:'characters',action:'create-character',target:'gen_stale',data:{name:'过期来源',kind:'npc'},reason:'测试'}]}));
    await click('status','生成资料草稿');await click('status','预览所选变更');await evaluate("ctx.chat[0].mes+=' 来源已修改';");await click('status','确认保存整组资料');
