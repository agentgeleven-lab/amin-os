...
// Run: node scripts/linkage-scroll-browser-smoke.mjs (AMIN_BROWSER / AMIN_SCROLL_ARTIFACTS optional).
...
const artifacts = process.env.AMIN_SCROLL_ARTIFACTS || path.resolve(root, '../linkage-scroll-browser-smoke');
...
try {
    ...
    await evaluate(`const service=(await import('/apps/linkage/service.js')).getSharedService();await service.saveSettings({...service.settings(),enabled:true});`);
    await evaluate("document.body.style.paddingBottom='1200px'");
    const measures = [];
    const metrics = () => evaluate(`(()=>{const d=document.querySelector('[data-placement="'+window.activePlacement+'"]'),b=d?.querySelector('#wsh-linkage-page'),p=d?.querySelector('[aria-label="统一条目预览内容"]'),r=p?.getBoundingClientRect(),br=b?.getBoundingClientRect();return {scrollX,scrollY,rootHeight:document.documentElement.scrollHeight,bodyHeight:document.body.scrollHeight,innerHeight,viewport:{height:visualViewport.height,offsetTop:visualViewport.offsetTop,pageTop:visualViewport.pageTop},bodyScroll:b?.scrollTop,bodyClient:b?.clientHeight,promptOpen:p?.closest('details')?.open,promptTop:r?.top,bodyTop:br?.top,bodyBottom:br?.bottom};})()`);
    for(const width of [1280,320,390]) {
      await send('Emulation.setDeviceMetricsOverride',{width,height:width<600?844:1000,deviceScaleFactor:1,mobile:width<600});
      for(const placement of ['os','floor']) {
        await evaluate(`document.querySelector('.wsh-dialog [aria-label="关闭"]')?.click();document.querySelector('.amin-drawer')?.setAttribute('hidden','');window.scrollTo(0,0);`);
        if(placement==='os') await app('status');
        else {await evaluate(`const b=[...document.querySelectorAll('.wsh-floor-button')].find(b=>b.dataset.statusMode==='current');if(b.getAttribute('aria-expanded')==='true')b.click();b.click();`);await waitFor(`!!document.querySelector('[data-placement="floor"]')`,'floor open');}
        await evaluate(`window.activePlacement='${placement}';window.scrollTo(0,0);const d=document.querySelector('[data-placement="${placement}"]');[...d.querySelectorAll('button')].find(b=>b.textContent==='状态栏')?.click();if('${placement}'==='floor')window.scrollTo(0,Math.max(0,d.getBoundingClientRect().top-20));`);await delay(100);
        const before=await metrics();
        await evaluate(`document.querySelector('[data-placement="${placement}"] [data-linkage-quick-copy]').click()`);await delay(160);
        const after=await metrics();measures.push({width,placement,before,after});
        const screenshot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});fs.writeFileSync(path.join(artifacts,`${placement}-${width}.png`),Buffer.from(screenshot.data,'base64'));
        if(!process.env.AMIN_SCROLL_BASELINE){for(const key of ['scrollX','scrollY','rootHeight','bodyHeight'])assert.equal(after[key],before[key],placement+' '+width+' '+key);assert.deepEqual(after.viewport,before.viewport);assert.equal(after.promptOpen,true);assert.ok(after.bodyScroll>0,'internal panel scrolled');}
      }
    }
    assert.deepEqual(errors,[]);assert.deepEqual(consoleErrors,[]);
    fs.writeFileSync(path.join(artifacts,process.env.AMIN_SCROLL_BASELINE?'baseline.json':'report.json'),JSON.stringify({passed:true,baseline:!!process.env.AMIN_SCROLL_BASELINE,measures,runtimeErrors:errors,consoleErrors},null,2));
    console.log('PASS '+measures.length+' scroll cases; '+JSON.stringify(measures.map(m=>({width:m.width,placement:m.placement,beforeY:m.before.scrollY,afterY:m.after.scrollY,pageHeight:m.after.rootHeight}))));await send('Browser.close').catch(()=>{});
} catch (error) {
