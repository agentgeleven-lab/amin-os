...
test('reply workspace opens settings without mounting a rewrite app and validates floor ownership',()=>{
 const ctx={chat:[{mes:'第一楼'}],chatMetadata:{},getCurrentChatId:()=> 'chat'};
 const target=new FakeNode('div');let mounts=0,disposals=0,opened;const source=ctx.chat[0];
 const options={getContext:()=>ctx,contextProvider:current=>floorContext(current,0,source),instanceId:'floor-settings',
  mountReply:opts=>{mounts++;assert.equal(opts.instanceId,'floor-settings');return {open(page){opened=page;},dispose(){disposals++;}};}};
 const api=mountWorkspace(target,options);assert.equal(mountWorkspace(target,options),api);assert.equal(mounts,1);
 api.open('settings');assert.equal(opened,'settings');api.open('rewrite');assert.equal(opened,'settings');
 assert.equal(api.acceptSource,undefined);ctx.chat[0]={mes:'已替换'};assert.throws(()=>api.open(),/楼层内容已变化/);
 api.dispose();api.dispose();assert.equal(disposals,1);assert.throws(()=>api.open(),/窗口已关闭/);
});
