...
For the matching existing studio, `studio-parallel.patch` documents the server update: 3 accepted jobs, simultaneous MiMo HTTP synthesis, one RVC conversion at a time. Back up app.py, compare/apply only to that version, then restart when idle. In-memory MiMo key must be entered again. Health reports batch_concurrency=3; older services are used serially.
