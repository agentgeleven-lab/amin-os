...
`metro.css` 管理小格布局，列数由保存的 4／6 列设置决定；`tile-desktop.js` 为每个磁贴写入明确的 `grid-column` / `grid-row` 起点和跨度，并根据容器更新正方形小格尺寸。不要添加 `dense` 或按手机宽度改变列数，也不要在 CSS 尺寸规则中覆盖保存的坐标。六种尺寸为 1×1、2×2、3×2、4×2、2×3、2×4。小磁贴同时显示图标和名称；长名称单行省略，完整名称仍通过可访问标签和标题提供。
