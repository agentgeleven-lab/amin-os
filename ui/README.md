...
`metro.css` 管理小格布局，列数由保存的 4／6 列设置决定；`tile-desktop.js` 为每个磁贴写入明确的 `grid-column` / `grid-row` 起点和跨度，并根据容器更新正方形小格尺寸。不要添加 `dense` 或按手机宽度改变列数，也不要在 CSS 尺寸规则中覆盖保存的坐标。八种尺寸为 1×1、2×1、1×2、2×2、3×2、4×2、2×3、2×4。小磁贴同时显示图标和名称；长名称单行省略，完整名称仍通过可访问标签和标题提供。
...
统一联动放在世界状态内部，采用 `ui-linkage.css` 的页面样式和已有 `amin-*` 控件。参与/读取/更新开关分列说明；模型更新必须有来源和整批预览，手动关联选择稳定实体 ID。手机统一使用单列表单、局部滚动的长资料及可横向滚动的分页。事实/记忆、日程/离场、外观/穿戴、关系建议、效果结算、楼层检查点继续使用所属应用。交互必须区分确认变更和失败后的重试保存。实际检查入口为 `scripts/linkage-browser-smoke.mjs`，它使用隔离模拟宿主和浏览器手机视口。

### 0.15 资料生成

五个应用与联合初始化共用 `apps/generation/view.js` 的折叠面板。来源默认不选，普通字段用于日常编辑，JSON 置于高级编辑。选择建议后先展示整组预览再确认；读取来源期间允许停止，保存失败使用幂等重试。浏览器检查入口为 `scripts/generation-browser-smoke.mjs`，模型与宿主使用测试替身。
