...
`metro.css` 管理六列固定小格布局；`tile-desktop.js` 为每个磁贴写入明确的 `grid-column` / `grid-row` 起点和跨度，并根据容器更新正方形小格尺寸。不要添加 `dense` 或按手机宽度改变列数，也不要在 CSS 尺寸规则中覆盖保存的坐标。六种尺寸为 1×1、2×2、3×2、4×2、2×3、2×4。小磁贴仅显示图标，名称继续通过可访问标签和标题提供。
